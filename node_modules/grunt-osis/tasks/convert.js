  ...
  grunt.registerMultiTask('convert', 'Convert from OSIS to Markdown', function() {
    ...
    if (srcExt === '.xml') {
        ...
        parse(srcFiles, options, function(err, result) {
          ...
          if (destExt === '.md') {
            var folder = f.dest.substring(0,f.dest.lastIndexOf(destExt));
            var b = result.osis.osisText.div; //books
            for(var i = 0; i < b.length; i++){
              var c = b[i].chapter;
              var basePath = folder + '/' + b[i].osisID.toLowerCase() + '/';
              var bookURL = '/'+ basePath.replace('data/bibles/gh-pages/','');
              var bp = bookURL.split('/');
              var languageID = bp[1];
              var bibleID = bp[2];
              ...
              for(var j = 0; j < c.length; j++){
                ...
                var chapterData = '---\n'
                  ...
                  + '### Chapter '+chapterNum+'\n'
                  + body;
                var chapterPath = basePath + chapterNum + destExt;
